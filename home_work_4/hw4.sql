...
-- transform our data 
WITH top_rated AS 
(SELECT movieid, AVG(rating) as avg_rating FROM movie.ratings GROUP BY movieid HAVING COUNT(*) > 50 ORDER BY movieid, avg_rating  DESC LIMIT 150)
SELECT top_rated.movieid, top_rated.avg_rating, genres.keywords INTO movie.top_rated_tags FROM top_rated 
INNER JOIN 
